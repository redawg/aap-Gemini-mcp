...
description: >-
  ...
  project or existing AAP (OpenShift / RHEL Podman), optionally installs Ansible
  Product Demos (install-apd.yml), then connects Cursor, Gemini CLI, Gemini Agent
  Platform, or the browser chat sandbox. Interview the user step by step (one
  decision at a time) based on their answers; ask for any missing inputs; on GCP
  optionally create RHEL 9+10 targets and a GCP dynamic inventory. Use for
  from-scratch GCP deploys, Podman AAP+MCP, APD seeding, MCP tokens, Cloud Run
  chatbot, or AAP MCP tool use.
---
...
- **Yes** → `CREATE_TARGET_HOSTS=yes` (VMs in Step 1b + dynamic inventory in Step 6b) → **Q9b**  
- **No** → AAP server only → **Q9b**

**Q9b — Install Ansible Product Demos (APD)?**  
> After AAP is up, run [`install-apd.yml`](https://github.com/ansible/product-demos/blob/main/install-apd.yml) to seed demo orgs/templates ([INSTALL-APD.md](../../../docs/INSTALL-APD.md)).

- **Yes** (default for labs) → `INSTALL_APD=yes` → **Q10**  
- **No** → skip APD → **Q10**

**Q10 — Ready to proceed?**  
Show a short summary (project, zone, DNS, registry, tarball, chat paths, write mode, targets, APD).  
...
**QE4** — Install APD? (default yes) → [INSTALL-APD.md](../../../docs/INSTALL-APD.md)  
**QE5** — Same as **Q7–Q9** (chat paths, write mode, targets if GCP available)  
**QE6** — Confirm plan → [DEPLOY-AND-CONNECT.md](../../../docs/DEPLOY-AND-CONNECT.md) / [CONNECT-GEMINI.md](../../../docs/CONNECT-GEMINI.md)
...
- [ ] 1c. If Q9b=yes: Install APD (INSTALL-APD.md / install-apd.yml)
...
0. Enable APIs → 1 AAP VM → 1b targets if Q9 → 2 DNS → 3 install → **3b APD if Q9b** → 4 token → 5 TLS → 6 write → 6b GCP inventory → 7–9 chat paths  
...
1. List job templates by name (expect APD templates if installed).  
...
5. (Write) Create group / launch an APD job template.  
...
4. **Q9b = yes** (default) → install APD via [INSTALL-APD.md](../../../docs/INSTALL-APD.md) after AAP is up.  
5. Prefer Path C for first chat demo; never invent URLs/tokens.  
